  ...
  auth()
    ...
    .catch((error) => {
      if (error.code === "auth/email-already-in-use") {
        Alert.alert("Error in SignUp", "Email already in use", [
          {
            text: "Ok",
          },
        ]);
      } else {
        Alert.alert("Something went wrong", "Something is causing problems", [
          {
            text: "Ok",
          },
        ]);
...
export function signIn(email: string, password: string, navigation: any) {
  auth()
    ...
    .catch((error) => {
      Alert.alert("Error in Login", "Email or Password incorrect", [
        {
          text: "Ok",
        },
      ]);
